...
| `policy.json` | 训练产物:两层策略 + Q 表 + 环境指纹 + 连接图指纹(三态:复用 / 增量微调 / 重训) |
...
- **策略复用/增量微调**:程序启动时算两类指纹——**环境指纹**(训练参数:状态/动作/奖励/仿真)+ **连接图指纹**(当前随机连接图哈希)。判定逻辑:
  - 两指纹都匹配 → **直接复用**(不训练)
  - 连接图变了、训练参数没变 → **增量微调**(在旧 Q 表上小学习率继续训,保留基础策略,快速适配新结构)
  - 无旧策略 / 训练参数变了 → **从零重训**
...
2. **策略持久化 + 增量探索**:`policy.json` 存两层策略 + Q 表 + 环境指纹 + 连接图指纹。先训练基础模型;之后遇到不同的随机连接图,在旧 Q 表上小学习率增量微调(保留基础策略),训练参数变化才全量重训——兼顾复现性与对新环境的适配。
