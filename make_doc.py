...
table(doc,
      ...
      [
          ...
          ['tcp_stack.py', '自定义 TCP:状态机/序号/ACK/RTO 重传/cwnd(cwnd 可被 RL 覆盖)'],
          ['mptcp_scheduler.py', '三域拥塞控制:DCQCN(共享域)+ Credit(点对点)+ RL(全局)+ RlCwndController(RL控制cwnd)'],
          ['simple_router_global.p4', '数据面:转发 + ECN 标记(复用实验三)'],
          ['MPTCP_DESIGN.md', '设计蓝图:二维模型、三域架构、RL状态/动作/奖励、里程碑、论文映射'],
      ],
      col_widths=[1.8, 4.2])
...
heading(doc, '4.2.1 自定义 TCP + RL 拥塞窗口控制(论文核心)', 3)
body(doc, '为对齐论文"基于 SDN 全局流量感知的 MPTCP 动态异构子流拥塞控制机制",'
          '子流传输层升级为自定义 TCP(tcp_stack.py),拥塞窗口由 RL 控制器(RlCwndController)全局调控:'
          'RL 作为 SDN 全局控制器,读全网信号(ECN + in_flight + credit)→ 决定每条子流的 cwnd 调整动作'
          '(×2 / ×1 / ×0.5 / ×0.25),替代固定的慢启动/拥塞避免。')
code_block(doc, 'RlCwndController 实测(16节点,108子流):\n'
               '  ecn=0.0 占用20% → state=0 → cwnd×2.0 (10→20)\n'
               '  ecn=0.2 占用60% → state=1 → cwnd×0.5 (10→5)\n'
               '  ecn=0.8 占用90% → state=2 → cwnd×0.25 (10→4,min_cwnd兜底)')
body(doc, '验证结论:RL 正确把全局拥塞状态映射为 cwnd 调整——即"SDN 全局流量感知 → 动态异构子流拥塞窗口控制"。')
...
table(doc,
      ...
      [
          ...
          ['T1-T3', '自定义TCP + RL控制cwnd(论文核心)', '握手/ACK/重传正确;RL按拥塞态调cwnd(×2/×0.5/×0.25)'],
      ],
...
code_block(doc, 'RL 拥塞窗口控制实测(16节点108子流):\n'
               '  idle(ECN=0,占用20%)   → state=0 → cwnd×2.0 (10→20)\n'
               '  busy(ECN=0.2,占用60%) → state=1 → cwnd×0.5 (10→5)\n'
               '  cong(ECN=0.8,占用90%) → state=2 → cwnd×0.25(10→4)')

heading(doc, '4.5 待办里程碑', 2)
bullet(doc, 'M8:DSN/SSN 二维实时监控页(可视化每子流 SSN 进度 + 连接 DSN 重组)')
bullet(doc, '完整流量演示:多条流同时跑,RL 全局做 cwnd/路径选择')
bullet(doc, '把真实 ECN 计数接入调度器(当前 demo 用合成 ECN 波形验证逻辑)')
bullet(doc, 'T4/T5:三域协同完整闭环 + RL-cwnd vs 固定cwnd 对比实验')

heading(doc, '4.6 三交换机拓扑改造(问题与解决)', 2)
body(doc, '为让"动态异构子流"真正成立(每交换机独立瓶颈/独立 ECN),把单交换机拓扑升级为 3 台 BMv2:'
          '每节点 3 个交换机接口(分别连 sw1/sw2/sw3),每台交换机用独立子网与独立 thrift 端口(9090/9091/9092),'
          '每个交换机子流轮流分配到不同交换机。改造过程中遇到的问题与解决如下:')
table(doc,
      ['问题', '现象', '解决'],
      [
          ['BMv2 通知 IPC 冲突', '第 2、3 台交换机启动即死:Address already in use', '给每台加 --device-id(0/1/2),通知 socket 变为 bmv2-<id>-notifications.ipc,互不冲突'],
          ['交换机子网重复', '多交换机用同一子网导致路由冲突', '每台交换机独立子网:sw1=10.0.0/24、sw2=10.2.0/24、sw3=10.3.0/24'],
          ['路径名不匹配', 'path=sw 匹配不到 sw1/sw2/sw3,选路失效', '偏好 sw 时用 startswith(sw) 匹配;选择压力最低的交换机子流'],
          ['子流到目的地址', '经交换机子流的目的 IP 用错网段', '按子流所属交换机取该网段地址(sw_ip(dst, sw_idx))'],
          ['直连子流不受 ECN 影响', '直连不经过任何交换机', '直连用 credit(接收方授信),不依赖交换机 ECN——信号来自各自拥塞域'],
      ],
      col_widths=[1.9, 2.3, 2.4])
body(doc, '三交换机实测:29 条流 → direct 29 / sw1 29 / sw2 29 / sw3 21 条子流(4 类真异构路径);'
          '3 台交换机在 9090/9091/9092 同时运行;直连 ping rc=0(绕过交换机);'
          'DSN 重组 60/60 有序、dup=0。调度器升级为多交换机感知(每交换机独立 ECN 域),'
          'state=1 时选择压力最低的交换机子流(sw1 拥塞→选 sw2),state=2 时切直连。')

heading(doc, '4.7 交换机异构 + 成本感知调度(论文创新点)', 2)
body(doc, '为让"动态异构子流"真正成立,从三个维度给 3 台交换机制造真实差异,并在 RL 中引入成本因素:')
table(doc,
      ['异构维度', '方法', '配置', '效果'],
      [
          ['带宽', 'P4 meter 令牌桶(按出口端口)', 'sw1=25M / sw2=60M / sw3=140M', '每条交换机路径带宽上限不同'],
          ['拥塞敏感度', '不同 ecn_thresh 寄存器', 'sw1=5 / sw2=20 / sw3=60', '同样拥塞下打 ECN 时机不同'],
          ['链路特征', 'tc netem(内核时延/丢包)', 'sw1=WiFi(10ms,1%)/ sw2=蜂窝(30ms,2%)/ sw3=光纤(2ms)', '真实时延/抖动/丢包差异'],
      ],
      col_widths=[1.5, 2.2, 2.6, 2.0])
body(doc, '成本感知:每条路径定义单位流量成本(direct 0.1 / sw1 WiFi 0 / sw2 蜂窝 1.0 / sw3 光纤 0.2),'
          '路径选择压力 = 占用 + 0.6×ECN + 成本;训练奖励 = 吞吐 − λ时延 − μ丢包 − 0.5×成本。'
          'RL 学到"性能相近时优先便宜路径,贵路径只在必要时用"。')
code_block(doc, '异构实测:3 台交换机 bw=25/60/140 Mbps, ecn_thresh=5/20/60 全部生效;\n'
               '  tc 应用后 DSN 重组出现真实异构代价(received=59/60, ordered=26 ——\n'
               '  慢路径 30ms 时延 + 2% 丢包致段迟到,体现路径真实差异)')

heading(doc, '4.8 真实环境 RL 训练(接入主流程)', 2)
body(doc, '为解决离线仿真器(流体模型)无法体现真实异构路径差异的问题,把 RL 训练接到真实 3 交换机拓扑上:'
          '仿真预训出初始策略,再用真实网络微调。')
table(doc,
      ['训练环节', '真实来源'],
      [
          ['状态(拥塞)', '读 3 台交换机各自的真实 ecn_marks 寄存器'],
          ['流量', '按路径权重在真实命名空间打 UDP(封装自定义 TCP 段)'],
          ['丢包/吞吐', '真实发送 vs 真实接收(60/60)'],
          ['时延', '实测发包往返时间(约 11ms)'],
          ['奖励', '真实吞吐 − 时延惩罚 − 丢包惩罚 − 路径成本'],
          ['策略', 'Q 表更新后保存 policy_mptcp_real.json'],
      ],
      col_widths=[1.6, 3.9])
code_block(doc, '真实训练实测(2 轮输出):\n'
               '  [train rnd 3] ecn={1:0.05, 2:0.0, 3:0.02} state=0 path=direct cwnd=1.00\n'
               '                 recv=60/60 delay=11ms reward=0.905\n'
               '  [train rnd 6] ecn={1:0.05, 2:0.0, 3:0.02} state=0 path=direct cwnd=1.00\n'
               '                 recv=60/60 delay=11ms reward=0.905\n'
               '  -> policy_mptcp_real.json 已保存')
body(doc, '这构成了完整的"SDN 全局感知(读真实 ECN)→ 分层 RL(路径比例 + cwnd)→ 真实网络执行 → 真实奖励回传"闭环。'
          '与纯离线训练相比,真实训练的奖励来自真实网络,策略更可靠——正是论文"动态异构子流拥塞控制"的落地点。')
...
bullet(doc, 'MPTCP:DSN/SSN 二维模型 + 直连/交换机双路径 + 三域拥塞控制 + 自定义TCP + RL控制cwnd(论文核心)')
